...
export default function UploadPanel() {
      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (status.data.stage === "done") {
            clearInterval(poll);
            setUploading(false);
            setMessage({ type: "success", text: `Upload complete. TX: ${status.data.result?.tx_hash?.slice(0, 18)}...` });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
      <h2 className="text-lg font-semibold text-gray-900">Upload & Configure</h2>
        ...
        {file ? (
          <div>
            {thumbLoading ? (
              ...
              <img src={preview} alt="Preview" className="mx-auto mb-2 max-h-32 rounded-lg object-contain" />
            ) : (
          ...
          <div className="text-sm text-gray-500">Drag & drop file here or click to browse</div>
        )}
      </div>
